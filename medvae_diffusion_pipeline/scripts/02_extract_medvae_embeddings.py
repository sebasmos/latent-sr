...
def encode_batch(model, images: torch.Tensor, backend: str) -> torch.Tensor:
    ...
    if backend == "medvae":
        ...
        # Force fp32 (disable surrounding autocast): medvae_4_4 specifically was
        # found producing NaN latents in ~86% of samples under fp16 autocast
        # (it loads with 8 missing channel_ds/channel_proj keys, unlike the other
        # medvae variants, which all encoded cleanly in fp16) - forcing fp32 for
        # every medvae variant is a strict-improvement safety net, not just 4_4.
        with torch.autocast(device_type=images.device.type, enabled=False):
            latents = model.encode(images.float())
    elif backend == "autoencoderkl":
        encoded = model.encode(images)
        latents = encoded.latent_dist.sample()
    elif backend == "klf4":
        # CompVis kl-f4 natural VAE (3x64x64), deterministic posterior mean.
        # Force fp32 (disable any surrounding autocast): the CompVis encoder loses
        # precision / can overflow under fp16, and we want fp32 latents that match
        # eval_klf4_ae.py (the AE ceiling) and the fp32 medvae/sd-vae latents.
        with torch.autocast(device_type=images.device.type, enabled=False):
            latents = model.encode(images.float())
    else:
...
def main():
    ...
    parser.add_argument(
        ...
        type=str,
        default="medvae",
        choices=["medvae", "sd-vae", "klf4"],
        help="Encoder backend: 'medvae' (default), 'sd-vae', or 'klf4' (CompVis natural VAE, 3x64x64)",
    )
    parser.add_argument(
        "--device",
        type=str,
    ...
    if args.backend == "sd-vae":
        ...
        sys.path.insert(0, str(Path(__file__).resolve().parents[2] / "slurm" / "revision"))
        from klf4_vae import load_klf4
        print(f"\nLoading CompVis KL-f4 VAE (natural, 3x64x64)...")
        model = load_klf4(device)
        model.requires_grad_(False)
        encoder_backend = "klf4"
        print("Encoder backend: klf4")
    else:
    ...
    # Input channels are a property of each MedVAE checkpoint's conv_in, NOT the
    # latent-channel suffix: e.g. medvae_4_4 has a 4-channel LATENT but a 1-channel
    # (grayscale) INPUT. Verified from model_weights/*.yaml `in_channels`:
    #   1-channel input: medvae_4_1_2d, medvae_8_1_2d, medvae_4_4_2d
    #   3-channel input: medvae_4_3_2d, medvae_8_4_2d
    # (The old "_1_" in name heuristic silently fed medvae_4_4 a 3-channel image.)
    MEDVAE_ONE_CHANNEL_INPUT = {"medvae_4_1_2d", "medvae_8_1_2d", "medvae_4_4_2d"}
    if args.backend == "medvae" and args.model_name in MEDVAE_ONE_CHANNEL_INPUT:
        n_channels = 1
        img_mode = "L"
    else:
        n_channels = 3
